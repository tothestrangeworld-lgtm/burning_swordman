...
'use server';
// =====================================================================
// 燃えろ剣士 - サーバー専用 DB アクセス層（Server Actions）
// =====================================================================
// ★ セキュリティ方針:
//   Supabase で RLS (Row-Level Security) を有効化したうえで、
//   サーバーサイドからは SUPABASE_SERVICE_ROLE_KEY（管理者権限キー）で
//   接続して RLS をバイパスする。これにより、
//     ・DB へのアクセスロジックは完全にサーバーへ隠蔽
//     ・Service Role Key は NEXT_PUBLIC_ を付けないためブラウザへ漏れない
//     ・公開鍵(anon)を握っても直接 DB を叩けない安全な状態
//   を実現する。
//
// ★ Next.js 制約:
//   'use server' ファイルは async 関数しか export できない。
//   型・定数・同期ヘルパーは types-api.ts / api.ts 側へ退避済み。
// =====================================================================
...
// =====================================================================
// Supabase クライアント（Service Role Key で初期化）
// -------------------------------------------------------------------
// ★ ここがセキュリティの核心:
//   NEXT_PUBLIC_ を付けない SUPABASE_SERVICE_ROLE_KEY を使用する。
//   このファイルは 'use server' のためサーバー上でのみ評価され、
//   キーがクライアントバンドルへ含まれることはない。
// =====================================================================
const SUPABASE_URL =
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  // サーバー起動時に環境変数の不足を即検知できるようにする。
  console.error(
    '[actions] SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY が設定されていません',
  );
}

// ★ サーバー専用クライアント（Service Role Key・RLSバイパス）。
//   セッション永続・自動リフレッシュは不要なので無効化する。
...
// =====================================================================
// 共通: Supabase エラーを統一例外へ変換するヘルパー（サーバー内専用）
// -------------------------------------------------------------------
// ★ 同期関数なので export しない（'use server' は async しか export 不可）。
// =====================================================================
...
// =====================================================================
// 共通: NextLevelInfo を組み立てる内部ユーティリティ（同期・非export）
// =====================================================================
...
export async function saveLogApi(
  ...
  if (acceptedTaskEvals.length > 0) {
    ...
    const { data: existingSelf, error: exErr } = await supabaseAdmin
      .from('task_logs')
      .select('task_id')
      .eq('user_id', user_id)
  ...
  if (acceptedTechniques.length > 0) {
    const { startIso, endIso } = resolveDayRange(date);
    const { data: existingTech, error: exTechErr } = await supabaseAdmin
      .from('technique_logs')
      .select('technique_id')
      .eq('user_id', user_id)
      .gte('date', startIso)
  ...
  if (taskRows.length > 0) {
    const { error } = await supabaseAdmin.from('task_logs').insert(taskRows);
  ...
  if (taskRows.length === 0 && techRows.length === 0) {
    const { data: stNow } = await supabaseAdmin
      .from('user_status')
  ...
  // --- xp_history への追記 ---
  if (earned > 0) {
    const { error: hisErr } = await supabaseAdmin.from('xp_history').insert({
      user_id,
      date: isoDate,
      type: 'gain',
      amount: earned,
...
export async function evaluateStudentApi(
  ...
  if (rows.length > 0) {
    const { error } = await supabaseAdmin.from('task_logs').insert(rows);
...
export async function evaluateBulkStudentsApi(
  ...
  for (const sid of student_ids) {
    try {
      ...
      const skipped = evaluations.filter((ev) =>
        already.has(ev.task_id),
      ).length;
...
export async function fetchDashboard(userId: string): Promise<DashboardData> {
  ...
  const [
    ...
    supabaseAdmin
      .from('task_logs')
      .select(
        'id, user_id, date, task_id, score, xp_earned, evaluator_id, comment',
      )
      .eq('user_id', userId)
      .gte('date', since30)
  ...
  const techniqueMaster: TechniqueMasterEntry[] = (
    techMasterRes.data ?? []
  ).map((t) => ({
  ...
  const titleMaster: TitleMasterEntry[] = (titleMasterRes.data ?? []).map(
    (t) => ({
      level: t.level,
      title: t.title,
    }),
  ...
  const { data: evalUsersRes } =
    evalIds.length > 0
      ? await supabaseAdmin.from('users').select('id, name').in('id', evalIds)
      : { data: [] };
  const evaluatorNameMap = new Map(
    (evalUsersRes ?? []).map((u) => [u.id, u.name]),
  );
  ...
  const taskLogs: TaskLogEntry[] = (taskLogsRes.data ?? []).map((l) => ({
    ...
    evaluator_name:
      l.evaluator_id && l.evaluator_id !== 'self'
        ? evaluatorNameMap.get(l.evaluator_id)
        : undefined,
    comment: l.comment ?? undefined,
  }));

  const techniqueLogs: TechniqueLogEntry[] = (techLogsRes.data ?? []).map(
    (l) => ({
      date: l.date,
...
export async function fetchTeacherDashboard(
  ...
  const statusMap = new Map((statusRes.data ?? []).map((s) => [s.user_id, s]));
  ...
  const techByUser = new Map<
    string,
    { T001: number; T002: number; T003: number }
  >();
  ...
  return {
    teacher: {
      ...
      grade:
        teacherRes.data.grade != null
          ? String(teacherRes.data.grade)
          : undefined,
    },
    students,
    titleMaster,
...
export async function fetchStudentDetail(
  ...
  return {
    student: {
      ...
      grade:
        studentRes.data.grade != null
          ? String(studentRes.data.grade)
          : undefined,
    },
    status,
    taskMaster,
...
export async function fetchMinigameStatusApi(
  ...
  return {
    ...
    locked: remaining <= 0,
    bestTimeMs:
      bestRes.data && bestRes.data.length > 0
...
export async function saveMinigameResultApi(payload: {
  user_id: string;
  averageTime: number;
  rank: MinigameRank;
...
// =====================================================================
// ★★★ Phase 7: あいことば（パスコード）変更 内部API ★★★
// =====================================================================

/**
 * 内部API: users.passcode を update する
 */
export async function updateUserPasscodeApi(payload: {
  user_id: string;
  new_passcode: string;
...
// =====================================================================
// 先生用: 課題マスター（task_master）の更新 内部API
// -------------------------------------------------------------------
// ★ 旧 updateTasks は getAuthUser に依存していたが、認証情報は
//   クライアント側（api.ts のラッパー）で判定し、ここでは純粋に
//   「先生として確定したデータ」を受け取って upsert するだけにする。
//   生徒呼び出しの no-op 判定はラッパー側で行う。
// =====================================================================
...
// =====================================================================
// ★★★ Phase 8: 「なかま」機能（門下生一覧＋応援システム）内部API ★★★
// =====================================================================
...
export async function fetchNakamaListApi(
  ...
  const [studentsRes, statusRes, titleRes, myCheersTodayRes] =
    await Promise.all([
      // ★ 自分以外の門下生（公開情報のみ）
      supabaseAdmin
        .from('users')
 ...
 */
export async function cheerStudentApi(payload: {
  from_user_id: string;
  to_user_id: string;
  ...
  // --- 相手の存在確認（名前取得も兼ねる） ---
  const { data: toUser, error: toErr } = await supabaseAdmin
    .from('users')
    .select('id, name, role')
    .eq('id', to_user_id)
    .single();
  ...
  const { error: insErr } = await supabaseAdmin.from('cheer_logs').insert({
    from_user_id,
    to_user_id,
    cheered_at: nowIso,
  });
  ...
  async function grantCheerXp(userId: string, reason: string): Promise<void> {
    const { data: st, error: stErr } = await supabaseAdmin
      .from('user_status')
      .select('total_xp')
      .eq('user_id', userId)
